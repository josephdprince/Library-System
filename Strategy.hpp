...
class Strategy {
    public:
        virtual std::string recommendation_algorithm(std::map<Book*, double>, std::vector<Book*>&, bool) = 0;
};
//This will go off of books user already likes
class Favorites : public Strategy {
    public:
	std::string str = "";
        virtual std::string recommendation_algorithm(std::map<Book*, double> list, std::vector<Book*>& library, bool key) {
